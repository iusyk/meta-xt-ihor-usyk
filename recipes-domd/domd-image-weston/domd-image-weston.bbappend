...
python __anonymous () {
    ...
    if product_name == "ihor-usyk" and not "domu" in d.getVar('XT_GUESTS_BUILD', True).split():
        d.appendVar("XT_QUIRK_BB_ADD_LAYER", "meta-aos")
}

SRC_URI = " \
    repo://github.com/iusyk/manifests;protocol=https;branch=ihor_prod;manifest=${XT_MANIFEST_FOLDER}/domd.xml;scmdata=keep \
"
